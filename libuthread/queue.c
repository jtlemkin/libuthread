...
int queue_iterate(queue_t queue, queue_func_t func) {
    ...
    while (curr_node) {
        queue->deletionOccured = 0;
        next_node = curr_node->next; // Store next node prior to editing.
        func(curr_node->data); // Call the function on the data
        if (!queue->deletionOccured) { // No deletion should have happened.
            if (curr_node->next != NULL) {
                curr_node = curr_node->next; // ...And we haven't reached the end. Proceed.
            } else {
                // Well, no deletion should have happened but curr_node->next is null. We have reached the end it seems.
                return 0;
            }
        } else {
            // A deletion has taken place. If the next node prior to deletion is still valid, move to it.
            if (next_node) {
                ...
                // called the function either. We must have deleted the final node.
                return 0;
            }
        }
    }
